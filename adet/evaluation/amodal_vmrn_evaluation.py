...
class AmodalVMRNEvaluator(DatasetEvaluator):
    """
    ...
    def reset(self):
        ...
        self._relationship_predictions = []
    ...
    def process(self, inputs, outputs):
        ...
        for i in range(len(inputs)):
            ...
            visible_prediction = {"image_id": input["image_id"]}
            occlusion_prediction = {"image_id": input["image_id"]}
            relationship_prediction = {"image_id": input["image_id"]}
            ...
            relationship_prediction["pred_rel_mat"] = pred_rel_mat.to(self._cpu_device)
            self._relationship_predictions.append(relationship_prediction)
    ...
    def evaluate(self):
        if self._distributed:
            ...
            self._relationship_predictions = comm.gather(self._relationship_predictions, dst=0)
            self._relationship_predictions = list(itertools.chain(*self._relationship_predictions))

            if not comm.is_main_process():
                return {}
        if len(self._amodal_predictions) == 0 or len(self._visible_predictions) == 0 \
            or len(self._occlusion_predictions) == 0 or len(self._relationship_predictions) == 0:
            self._logger.warning("[Amodal_VMRNEvaluator] 1st Did not receive valid predictions.")
            return {}
        ...
        file_path = os.path.join(self._output_dir, "instances_relationship_predictionss.pth")
        with PathManager.open(file_path, "wb") as f:
            torch.save(self._relationship_predictions, f)
        ...
        if "pred_rel_mat" in self._relationship_predictions[0]:
            self._eval_rel_matrix(self._relationship_predictions)

        # Copy so the caller can do whatever with results
        return copy.deepcopy(self._results)


    def _eval_rel_matrix(self, predictions):
        """
        Evaluate the relationship matrix in predictions.
        Fill self._results with the metrics for "box_proposals" task.
        """
        ## get GT
        coco_api_eval = copy.deepcopy(self._coco_api)
        gts = []
        for key, ann in coco_api_eval.imgs.items():
            gt = coco_api_eval.imgs[key]["rel_mat"]
            gts.append(gt)
        ...
        for pred, gt in zip(predictions, gts):
            p, g = np.array(pred['pred_rel_mat']), np.array(gt)
            # print(np.unique(p), np.unique(g))
            ...
            if tp + fn + fp + tn: ac = (tp + tn) / (tp + fn + fp + tn)
            else: continue
            
            if tp + fp: pr = tp / (tp + fp)
            else: pr = 0

            if tp + fn: re = tp / (tp + fn)
            else: re = 0
        ...
        recall /= total
        f1score /= total
        print('acc', accuracy, 'prec', precision, 'reca', recall, 'f1', f1score)
        ...
        self._results["pred_rel_mat"] = res
